...
setwd("D:\\E\\博士\\R_程序\\UCEC\\GSE63678")

## 读入基因表达数据
exprSet <- read.table("GSE63678_series_matrix.txt",header=T,sep='\t',fill=TRUE,strip.white = T)
# View(exprSet[,1:2])
...
## 读入芯片探针注释文件
anno <- read.table("GPL571.txt",header=T,sep='\t',fill=TRUE,strip.white = T,quote = "")  # 探针和基因ID的对应文件
# View(anno[,1:11])

library(dplyr)
library(tidyr)
anno2 <- anno %>%     # GSE73685_family.soft 文件的内容 
  select(ID,Gene.ID) %>%             # 提取文件中 ID,Gene_ID 两列 
  filter(Gene.ID != '') 
...
dim(expset4)   # 13749    35
# write.table(expset4,"GSE63678_expr.txt",quote=F,sep="\t")  


# 添加 0/1 标签 ---------------------------------------------------------------

lable = read.csv("GSE63678_all.csv", header = T, sep=',')
# View(lable)
lable_1 <- lable[which(lable$Tissue == "endometrium"),]
# View(lable2)
expset5 <- expset4[,which(lable$Tissue == "endometrium")]
dim(expset5)          # 13749    12  
# View(expset5[1:10,])

lable2 = read.csv("GSE63678_ucec.csv", header = T, sep=',')
dim(lable2)    # 17   2
data = rbind(as.matrix(t(lable2[,2])), as.matrix(expset5))
rownames(data) <- c('Lable', rownames(expset5))
View(data[1:10,])
dim(data)    # 13750    12
# write.table(data,"GSE63678_outcome_ucec.txt",quote=F,sep="\t")   
...
## 读入数据
data = read.table("GSE63678_outcome_ucec.txt", header = T, sep='\t', fill=TRUE, strip.white = T, check.names = F)
...
data1 <- my_scale(data)
dim(data1)    # 13750    12
# write.table(data1,"GSE63678_scale_ucec.txt",quote=F,sep="\t")  
